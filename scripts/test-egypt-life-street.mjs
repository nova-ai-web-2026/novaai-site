...
try{
 ...
 assert.ok(await page.evaluate(()=>{const scene=BABYLON.Engine.LastCreatedEngine.scenes[0],head=scene.getMeshByName(window.__EGYPT_CAST.actors.hero.head);return head.isEnabled()&&BABYLON.Vector3.Distance(scene.activeCamera.position,head.getAbsolutePosition())<6;}),'waking actor is missing');
 ...
 await page.click('#storyNext'); // Let the next line type while muted.
