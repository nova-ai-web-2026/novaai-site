(() => {
  ...
  function loadV115(){
    ...
    const s=document.createElement('script');s.src='game-v11-4-audio.js?v=11.5';s.dataset.egyptV114Audio='true';s.async=false;s.onerror=()=>console.error('V11.5 isolated audio failed to load');document.body.appendChild(s);
  }
  function loadV112(){
    if(document.querySelector('script[data-egypt-v112-audio]')){loadV115();return;}
    const s=document.createElement('script');s.src='game-v11-2-audio.js?v=11.3';s.dataset.egyptV112Audio='true';s.async=false;s.onload=loadV115;s.onerror=()=>{console.error('V11.3 calm audio failed to load');loadV115();};document.body.appendChild(s);
  }
  async function boot(){
    installStartupGuard();
    for(let i=0;i<260;i++){if(window.__V111_PATCH?.version==='11.1')break;await sleep(50);}
    const enforce=()=>{const ctx=window.__V11_AUDIO_CONTEXT;if(!ctx)return;const oldV11=window.__V11_OWN_MASTER;if(oldV11&&!oldV11.__v111Disconnected){try{oldV11.disconnect();oldV11.__v111Disconnected=true;}catch(_){}}try{if(oldV11)oldV11.gain.setTargetAtTime(0,ctx.currentTime,.02);}catch(_){}try{if(window.__V11_LEGACY_MASTER)window.__V11_LEGACY_MASTER.gain.setTargetAtTime(0,ctx.currentTime,.025);}catch(_){}try{window.__V112_MASTER?.disconnect();}catch(_){}};
    const timer=setInterval(enforce,80);enforce();
    window.__V11_AUDIOFIX={version:6,legacyLocked:true,v11BusDisconnected:true,legacyCoreMuted:true,startupGuard:true,isolatedV115:true,intervalMs:80};
