(() => {
 ...
 function label(name,text,x,y,z,parent){
  ...
  const mat=new B.StandardMaterial('street24_label_'+name,ctx.scene);mat.diffuseTexture=t;mat.specularColor=B.Color3.Black();
  for(const side of [-1,1]){const m=B.MeshBuilder.CreatePlane('street24_label_'+name+side,{width:.65,height:.24},ctx.scene);m.position.set(x,y,z+side*.035);m.rotation.y=side<0?0:Math.PI;m.material=mat;m.isPickable=false;m.metadata={readableArabic:true,mount:'signal-post',supports:[parent.name]};}
 }
 ...
 function buildSignals(){
  ...
  for(const c of crossings.filter(c=>c.signal))for(const side of [-1,1]){
   ...
   box('pedestrianHousing_'+c.id+side,.37,.72,.19,x,2.16,z,'#202824');label(c.id+side,'للمشاة',x,1.64,z,post);
   for(const [color,y] of [['red',2.35],['green',2.04]]){const m=B.MeshBuilder.CreateSphere('street24_'+color,{diameter:.23,segments:12},ctx.scene);m.position.set(x,y,z-.09);m.scaling.z=.65;const mat=new B.StandardMaterial('street24_bulb_'+m.uniqueId,ctx.scene);mat.diffuseColor=B.Color3.FromHexString(color==='red'?'#df483c':'#42c883');m.material=mat;m.isPickable=false;bulbs.push({m,mat,color,signal:c.signal});}
  }
 ...
 async function install(){
  ...
  window.addEventListener('egypt-story-finished',()=>{ctx.camera.position.set(-30.3,1.72,-30.3);begin(true);});
