(() => {
  ...
  const pools={},cursors={},playsByKey={},resolvedByKey={},rejectedByKey={};
  ...
  const state=()=>({
    version:'11.8',engine:'sampled-media-sfx',installed:true,mediaUnlocked,unlockAttempted,unlockFailures,startCuePlayed,
    sampleFiles:Object.keys(files).length,playCalls,playResolved,playRejected,playsByKey:{...playsByKey},resolvedByKey:{...resolvedByKey},rejectedByKey:{...rejectedByKey},
  ...
  function unlockMedia(){
    ...
    unlockPromise=(async()=>{
      ...
      unlockFailures=results.filter(v=>!v).length;mediaUnlocked=results.some(Boolean);publish();return mediaUnlocked;
  ...
  function playSample(key,volume=.75,rate=1){
    ...
    try{
      ...
      const callId=playCalls;lastPlayed={key,at:performance.now(),playCalls:callId,promiseResolved:false,readyState:a.readyState,currentTime:a.currentTime,volume:a.volume,rate};publish();
      const p=a.play();
      if(p&&typeof p.then==='function')p.then(()=>{
        ...
        playRejected++;rejectedByKey[key]=(rejectedByKey[key]||0)+1;
        if(lastPlayed?.playCalls===callId)lastPlayed.error=String(err);publish();
      });
      return true;
    }catch(err){playRejected++;rejectedByKey[key]=(rejectedByKey[key]||0)+1;lastPlayed={key,at:performance.now(),playCalls,error:String(err)};publish();return false;}
  }

  function step(kind='pavement'){leftFoot=!leftFoot;events.step++;playSample(kind==='asphalt'?'step_asphalt':'step_pavement',kind==='asphalt'?.78:.72,leftFoot?.97:1.03);publish();}
  ...
  window.__V118_MEDIA_POOLS=pools;window.__V118_SFX_API={play,unlockMedia,state,playSample};
