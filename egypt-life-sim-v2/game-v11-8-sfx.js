(() => {
  ...
  function refreshRefs(){
    ...
    return !!(ctx&&parentMaster&&bus&&analyser);
  }
  ...
  function noiseBuffer(seconds=.1, decay=4, brown=.14){
    ...
    for(let i=0;i<len;i++){
      ...
      const env=Math.pow(1-i/len,decay);d[i]=(low*.62+white*.38)*env;
    }
    return b;
  }
  ...
  function burst({freq=500,q=.8,vol=.15,dur=.08,type='bandpass',delay=0,highpass=0}={}){
    ...
    src.buffer=noiseBuffer(Math.max(.045,dur*1.7),3.3,.11);f.type=type;f.frequency.value=freq;f.Q.value=q;
  ...
  function step(kind='pavement'){
    ...
    burst({freq:asphalt?980:1280,q:.9,vol:.07,dur:.045,type:'bandpass',delay:.008,highpass:500});window.__V118_SFX=state();
  }
  ...
  function hookModals(){
    for(const [id,fn] of [['shop',shopOpen],['dialog',shopOpen]]){
      ...
      new MutationObserver(()=>{const now=getComputedStyle(el).display!=='none';if(now&&!visible)ensureAudio().then(ok=>ok&&fn());visible=now;}).observe(el,{attributes:true,attributeFilter:['style','class']});
    }
  }
  ...
  function hookToast(){
    ...
    new MutationObserver(()=>{
      ...
      ensureAudio().then(ok=>{if(!ok)return;if(text.startsWith('اشتريت '))buy();else if(text.includes('خلصت طلبية')&&text.includes('كسبت'))reward();else if(text.includes('رجعت بيت العيلة'))door();else if(text.includes('الفلوس مش مكفية'))deny();else if(text.includes('بدأت يوم جديد')||text.includes('رجعت لآخر مكان محفوظ'))startCue();else if(text.includes('لفّيت في سوق الحارة')){burst({freq:720,q:.7,vol:.11,dur:.10});burst({freq:1250,q:.8,vol:.07,dur:.07,delay:.07});}});
  ...
  function hookFootsteps(){
    stepTimer=setInterval(()=>{
      ...
      if(lastCam){const delta=Math.hypot(cam.x-lastCam.x,cam.z-lastCam.z);if(delta<2.5){stepTravel+=delta;while(stepTravel>.62){const road=Math.min(...[-72,-24,24,72].map(r=>Math.min(Math.abs(cam.x-r),Math.abs(cam.z-r))));step(road<4.8?'asphalt':'pavement');stepTravel-=.62;}}else stepTravel=0;}
