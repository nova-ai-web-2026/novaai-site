(() => {
  ...
  const stableMenu=()=>{
    ...
    if(foot&&foot.textContent!==`V${document.documentElement.dataset.release||'11.23.0'} — مشوار الفطار — شنطة وحفظ وحوار العيلة`)foot.textContent=`V${document.documentElement.dataset.release||'11.23.0'} — مشوار الفطار — شنطة وحفظ وحوار العيلة`;
  ...
  function markReady(){
    if(ready)return;
    if(!window.__V12_PROLOGUE?.ready||!window.__EGYPT_PEOPLE?.ready||!window.__EGYPT_CAST?.ready||!window.__EGYPT_FRONTAGES?.ready||(document.querySelector('script[src^="game-v11-23-quality.js"]')&&!window.__EGYPT_QUALITY?.ready)){setTimeout(markReady,100);return;}ready=true;window.__V119_READY=true;window.__V118_READY=true;window.__V117_READY=true;window.__V116_READY=true;menuObserver?.disconnect();stableMenu();attachDebug();setTimeout(attachDebug,0);setTimeout(attachDebug,500);
