(() => {
  ...
  const audioState=()=>({...window.__V119_AUDIO,version:VERSION,ctxState:ctx?.state||null,masterValue:master?.gain?.value??null,gameMasterValue:gameMaster?.gain?.value??null,gameMasterConnected:!!gameMaster,legacyBusCount,ready,queuedStart});
  const attachDebug=()=>{
  ...
  const stableMenu=()=>{
    ...
    const t='اصحى، هات الفطار، وخلي بالك من الناس والعربيات. استنى إشارة المشاة وعدّي بالراحة.';
    if(tagline&&tagline.textContent!==t)tagline.textContent=t;
    if(foot&&foot.textContent!==`V${document.documentElement.dataset.release||'11.24.0'} — مشوار الفطار — شنطة وحفظ وحوار العيلة`)foot.textContent=`V${document.documentElement.dataset.release||'11.24.0'} — مشوار الفطار — شنطة وحفظ وحوار العيلة`;
  ...
  function fail(msg,err){
    ...
    window.__V119_AUDIO={version:VERSION,started:false,error:String(err||msg)};
  ...
  function markReady(){
    if(ready)return;
    if(!window.__V12_PROLOGUE?.ready||!window.__EGYPT_PEOPLE?.ready||!window.__EGYPT_CAST?.ready||!window.__EGYPT_FRONTAGES?.ready||(document.querySelector('script[src^="game-v11-23-quality.js"]')&&!window.__EGYPT_QUALITY?.ready)||(document.querySelector('script[src^="game-v11-24-street-life.js"]')&&!window.__EGYPT_STREET_LIFE?.ready)||(document.querySelector('script[src^="game-v11-24-home.js"]')&&!window.__EGYPT_HOME24?.ready)){setTimeout(markReady,100);return;}ready=true;window.__V119_READY=true;window.__V118_READY=true;window.__V117_READY=true;window.__V116_READY=true;menuObserver?.disconnect();stableMenu();attachDebug();setTimeout(attachDebug,0);setTimeout(attachDebug,500);
