(() => {
  ...
  const DEFAULT_STATE = {money:300,hunger:84,energy:92,mood:76,minute:8*60+20,day:1,task:0,worked:0,savedX:-18,savedZ:-18};
  ...
  function addBalcony(x,front,y,w,seed){
    ...
    const posts=Math.max(2,Math.floor(bw/.75));
    for(let i=0;i<=posts;i++){const px=x-bw/2+i*(bw/posts);box('railPost',.045,.6,.045,px,y-.43,front-1.21,rail,false);}
  ...
  function addGroundShops(x,front,w,seed){
    ...
    for(let s=0;s<count;s++){
      ...
      for(let i=-2;i<=2;i++){const st=box('awningStripe',.18,.02,.92,sx+i*(sw-.8)/5,2.66,front-.77,mat('stripeA',i%2?'#f0dfbf':'#6f4b34'),false);st.rotation.x=.10;}
  ...
  function addRoof(x,z,w,d,h,seed){
    ...
    if(seed%2===0){cyl('dishPole',.06,.65,x+w*.21,h+.85,z-d*.15,metal,8);const dish=BABYLON.MeshBuilder.CreateDisc('dish',{radius:.68,tessellation:18},scene);dish.position.set(x+w*.21,h+1.25,z-d*.15);dish.rotation.x=1.05;dish.rotation.z=seed*.08;dish.material=mat('dish','#d6d0c5');dish.isPickable=false;}
  }
  ...
  function buildMarket(){
    const x=48,z=-48;sign('سوق الحارة',x,4.1,z+14,7.4,1,'#8a4d2d',0);
    for(let i=0;i<4;i++){const sx=x-12.5+i*8.4;box('stall',6.0,.72,3.4,sx,.38,z+2.5,mat('stall','#735139'),true);box('shade',6.25,.12,4.0,sx,2.75,z+2.5,mat('shade'+i,i%2?'#c79b3e':'#b94b3b'),false);decorateProduce(sx,z+.4);}
    const hot=box('marketHot',30,2.4,22,x,1.2,z,mat('hot','#fff'),false);hot.visibility=0;world.interactables.push({mesh:hot,kind:'market',name:'سوق الحارة',x,z});
  }
  function buildAhwa(){const x=-48,z=48;sign('قهوة المعلم فتحي',x,3.7,z-13,7,.9,'#5d3923',0);for(let r=0;r<2;r++)for(let c=0;c<4;c++){const px=x-6.5+c*4.2,pz=z-6.5+r*4;const t=cyl('table',1,.72,px,.36,pz,mat('table','#725033'),10);t.checkCollisions=false;for(const ox of [-.9,.9])box('chair',.5,.72,.5,px+ox,.36,pz,mat('chair','#68472f'),false);}}
  function buildLandmarks(){
    const h=box('homeDoor',2.1,2.8,.25,-96,1.4,-82.5,mat('door','#553622'),false);sign('بيت العيلة',-96,3.35,-82.7,3.2,.72,'#553622');world.home={mesh:h,kind:'home',name:'بيت العيلة',x:-96,z:-82.5};world.interactables.push(world.home);
    const j=box('jobBooth',3.7,2.3,1.9,-16,1.15,64,mat('job','#245d66'),true);sign('طلبات الحارة',-16,2.85,63,4.2,.8,'#245d66');world.job={mesh:j,kind:'job',name:'شغل التوصيل',x:-16,z:64};world.interactables.push(world.job);
    buildFulCart(-8,-8);
    box('mosque',13,6,11,91,3,91,plasterMat('mosque','#d6ccb5',91),true);const dome=BABYLON.MeshBuilder.CreateSphere('dome',{diameter:5.4,segments:16,slice:.55},scene);dome.position.set(91,7.1,91);dome.material=mat('dome','#819781');cyl('minaret',1.55,13,97,6.5,92,plasterMat('minaret','#d6c8aa',92),12);
  }
  ...
  function makeVehicle(type,i){
    ...
    const hit=box('vehCollider',w*.92,Math.max(.9,h),d*.88,0,.18,0,mat('hot','#ffffff'),true);hit.visibility=0;hit.parent=root;
  ...
  function enterGame(newGame){if(newGame){localStorage.removeItem(SAVE_KEY);resetState();}else{state=loadState();camera.position.set(state.savedX,PLAYER_EYE,state.savedZ);yaw=camera.rotation.y=0;pitch=camera.rotation.x=0;updateHUD();}ui.menu.style.display='none';ui.menuStatus.textContent='';startAudio();if(!isTouch)canvas.requestPointerLock?.();showToast(newGame?'بدأت يوم جديد في الحارة 🇪🇬':'رجعت لآخر مكان محفوظ');}
  ...
  window.__egyptDebug={
    ...
    resetPose:()=>{camera.position.set(DEFAULT_STATE.savedX,PLAYER_EYE,DEFAULT_STATE.savedZ);yaw=0;pitch=0;camera.rotation.set(0,0,0);}
  };
