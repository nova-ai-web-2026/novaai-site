(() => {
  ...
  function addShops(x,front,w,seed){
    ...
    for(let s=0;s<count;s++){
      const sx=x-w/2+sw*(s+.5),data=shopData[Math.abs(seed+s)%shopData.length];box('shopFrame',sw-.28,2.58,.24,sx,1.4,front-.15,mat('shopFrame','#392f28'));box('shopGlass',sw-.62,2.16,.1,sx,1.34,front-.3,mat('shopGlass','#26373b','#071012'));
  ...
  function update(){
    const dt=Math.min(engine.getDeltaTime(),45)/16.6667,active=ui.menu.style.display==='none'&&!modal;if(active){updateMovement(dt);updateNeeds(dt);updateVehicles(dt);updatePeople(dt);updateInteraction();updateAudio();}updateDayNight();updateHUD();drawMap();const now=performance.now();if(now-lastSave>12000&&ui.menu.style.display==='none'){saveState();lastSave=now;}
  }
  ...
  function closeModals(){if(modal)emitSfx('close');modal=false;ui.shop.style.display='none';ui.dialog.style.display='none';}
  ...
  function enterGame(newGame){if(newGame){localStorage.removeItem(SAVE_KEY);resetState();}else{state=loadState();camera.position.set(state.savedX,EYE,state.savedZ);yaw=0;pitch=0;camera.rotation.set(0,0,0);updateHUD();}ui.menu.style.display='none';ui.menuStatus.textContent='';startAudio();if(!TOUCH)canvas.requestPointerLock?.();showToast(newGame?'بدأت يوم جديد في الحارة 🇪🇬':'رجعت لآخر مكان محفوظ');}
