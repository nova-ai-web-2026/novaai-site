(() => {
  ...
  window.__egyptDebug={getCamera:()=>({x:camera.position.x,y:camera.position.y,z:camera.position.z,yaw,pitch,fov:camera.fov,ellipsoidY:camera.ellipsoid.y,offsetY:camera.ellipsoidOffset.y}),applyLook:(dx,dy)=>{applyLook(dx,dy);camera.rotation.x=pitch;camera.rotation.y=yaw;},audioState:()=>({enabled:audio.enabled,state:audio.ctx?.state||'none'}),resetPose:()=>{keys.clear();joyX=joyY=0;joyPointer=lookPointer=null;running=false;ui.knob.style.transform='translate(0,0)';camera.position.set(DEFAULT.savedX,EYE,DEFAULT.savedZ);yaw=0;pitch=0;camera.rotation.set(0,0,0);}};
