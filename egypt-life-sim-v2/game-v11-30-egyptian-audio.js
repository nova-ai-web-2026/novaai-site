(() => {
  ...
  // Kill any older sampled menu players before creating the one owner.
  ...
  function restoreGameplayAudioContext(){
    if(!started())return;
    try{window.__EGYPT_MENU_AUDIO_RESTORE_CONTEXT?.();}catch(_){}
  }
  ...
  // Musical button sting is intentionally interaction-mode only. In continuous mode
  // it would sit on top of the loop and sound like a doubled copy.
  function playSting(){
