(() => {
  ...
  const launchGame=engineSource=>{
    if(window.__SHWARE3_MODULE_LOADING)return;window.__SHWARE3_MODULE_LOADING=true;window.__SHWARE3_ENGINE_SOURCE=engineSource;if(status)status.textContent='جاري تجهيز شوارع النور V3.2…';
    const module=document.createElement('script');module.type='module';module.src=`./src/game.js?v=${BUILD}`;module.onerror=()=>showFatal('ملفات اللعبة اتحملت ناقصة. اعمل إعادة تحميل للصفحة.');document.body.appendChild(module);
