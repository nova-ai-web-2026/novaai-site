...
import {createWorld,setDoorOpen,updatePedestrians,updateTraffic,updatePolice} from './world.js';
...
function bindMobileControls(){
  const joy=$('joy'),knob=$('knob'),act=$('act'),run=$('run');
  if(!joy||!knob||!act||!run)return;
  const updateJoy=e=>{
    const r=joy.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,max=r.width*.34;
    let dx=e.clientX-cx,dy=e.clientY-cy;const mag=Math.hypot(dx,dy)||1;
    if(mag>max){dx=dx/mag*max;dy=dy/mag*max;}
    touch.x=Math.max(-1,Math.min(1,dx/max));touch.z=Math.max(-1,Math.min(1,-dy/max));
  ...
  joy.addEventListener('pointerdown',e=>{e.preventDefault();touch.pointer=e.pointerId;try{joy.setPointerCapture(e.pointerId);}catch{}updateJoy(e);});
  joy.addEventListener('pointermove',e=>{if(touch.pointer===e.pointerId){e.preventDefault();updateJoy(e);}});
  ...
  const runOn=e=>{e.preventDefault();touch.run=true;run.classList.add('pressed');try{run.setPointerCapture(e.pointerId);}catch{}};
  const runOff=e=>{e.preventDefault();touch.run=false;run.classList.remove('pressed');};
  run.addEventListener('pointerdown',runOn);run.addEventListener('pointerup',runOff);run.addEventListener('pointercancel',runOff);run.addEventListener('lostpointercapture',runOff);
  act.addEventListener('pointerdown',e=>{e.preventDefault();act.classList.add('pressed');if(started)interact();try{act.setPointerCapture(e.pointerId);}catch{}});
  const actOff=e=>{e.preventDefault();act.classList.remove('pressed');};
  act.addEventListener('pointerup',actOff);act.addEventListener('pointercancel',actOff);act.addEventListener('lostpointercapture',actOff);
}
bindMobileControls();
...
function inputAxes(){
  ...
  let x=Math.max(-1,Math.min(1,kx+touch.x)),z=Math.max(-1,Math.min(1,kz+touch.z));
  const len=Math.hypot(x,z);if(len>1){x/=len;z/=len;}return{x,z};
}
...
function updateMission(){
  const p=inVehicle?world.driveCar.position:world.player.position,step=mission.current()?.id;if(!step||mission.failed||mission.completed)return;
  if(step==='leave_apartment'&&p.z>-10.15){mission.advance(step);showToast('تمام. دلوقتي روح للمحل.');}
  ...
  else if(step==='cross_safely'&&p.z>4.2&&Math.abs(p.x-WORLD.crossingX)<5){if(trafficSystem.pedestrianGreen){mission.advance(step);showToast('عدّيت بأمان. كمل للمحل.');}else{mission.fail('عدّيت والإشارة مش سامحة');showToast('المهمة فشلت — عدّيت والإشارة مش سامحة',4);}}
  else if(step==='enter_shop'&&p.z>9.4&&p.z<17&&p.x>14.5&&p.x<25.5){mission.advance(step);showDialogue('عم صابر','صباح الفل يا باشا، أطلبلك إيه؟',4);showToast('اتكلم مع عم صابر');}
  else if(step==='return_home'&&dist2D(p,WORLD.homeReturn)<3.2){mission.advance(step);economy.earn(MISSION.reward);showToast(`المهمة خلصت — كسبت ${MISSION.reward} جنيه`,4);saveGame(false);}
}

function nearestInteraction(){
  const p=inVehicle?world.driveCar.position:world.player.position;if(inVehicle)return {type:'car_exit'};
  const candidates=[{type:'apt_door',d:dist2D(p,world.aptDoor.position)},{type:'shop_door',d:dist2D(p,world.shopDoor.position)},{type:'cashier',d:dist2D(p,world.cashier.position)},{type:'car',d:dist2D(p,world.driveCar.position)}].filter(v=>v.d<2.25).sort((a,b)=>a.d-b.d);return candidates[0]||null;
}
function interact(){
...
function openShop(){
  if(mission.current()?.id!=='buy_breakfast'&&!breakfastBought){showDialogue('عم صابر','لفّ شوية في المحل الأول وتعالى لي.',3);return;}
...
function updatePrompt(){
  const i=nearestInteraction();if(!i){hidePrompt();return;}
  if(i.type==='car')showPrompt(STRINGS.enterCar);else if(i.type==='car_exit')showPrompt(STRINGS.exitCar);
  else if(i.type==='apt_door'||i.type==='shop_door')showPrompt(i.type==='apt_door'?(world.aptDoor.metadata.open?'اضغط E تقفل باب الشقة':'اضغط E تفتح باب الشقة'):(world.shopDoor.metadata.open?'اضغط E تقفل باب المحل':'اضغط E تفتح باب المحل'));
  else if(i.type==='cashier')showPrompt('اضغط E واتكلم مع عم صابر');
}
...
window.__SHWARE3_DEBUG={
  state:()=>({ready:!!window.__SHWARE3_READY,started,inVehicle,touchDevice,engineSource:window.__SHWARE3_ENGINE_SOURCE,player:{x:world.player.position.x,y:world.player.position.y,z:world.player.position.z},mission:mission.current()?.id||null,meshes:scene.meshes.length}),
  interact:()=>interact()
};
