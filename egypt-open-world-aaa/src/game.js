...
window.addEventListener('keyup',e=>keys.delete(e.code));window.addEventListener('resize',()=>engine.resize());
const dist2D=(a,b)=>Math.hypot(a.x-b.x,a.z-b.z);const arabicDigits=v=>String(v).replace(/\d/g,d=>'٠١٢٣٤٥٦٧٨٩'[d]);
...
const showNpcLine=(name,text)=>showDialogue(name,text,2.2);function hidePrompt(){$('prompt').classList.remove('show');$('prompt').textContent='';}function showPrompt(text){$('prompt').textContent=text;$('prompt').classList.add('show');}
...
function playerMove(dt){
  ...
  if(inVehicle){const car=world.driveCar,accel=(keys.has('KeyW')||keys.has('ArrowUp')?1:0)-(keys.has('KeyS')||keys.has('ArrowDown')?1:0),steer=(keys.has('KeyA')||keys.has('ArrowLeft')?1:0)-(keys.has('KeyD')||keys.has('ArrowRight')?1:0);car.metadata.speed+=(accel*8-car.metadata.speed*1.8)*dt;car.metadata.speed=Math.max(-4,Math.min(9,car.metadata.speed));if(Math.abs(car.metadata.speed)>.2)car.metadata.heading+=steer*dt*1.6*Math.sign(car.metadata.speed);const dir=new B.Vector3(Math.sin(car.metadata.heading),0,Math.cos(car.metadata.heading));car.moveWithCollisions(dir.scale(car.metadata.speed*dt));car.rotation.y=car.metadata.heading;p.position.copyFrom(car.position);camera.lockedTarget=car;return;}
  const x=(keys.has('KeyD')||keys.has('ArrowRight')?1:0)-(keys.has('KeyA')||keys.has('ArrowLeft')?1:0),z=(keys.has('KeyW')||keys.has('ArrowUp')?1:0)-(keys.has('KeyS')||keys.has('ArrowDown')?1:0),running=keys.has('ShiftLeft')||keys.has('ShiftRight'),speed=running&&stamina>1?5.2:3.1;
